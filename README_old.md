...
### cmd_vel Bridge — Startup Cheat Sheet

Bridges `/commands/velocity` from the laptop (Jazzy) to baymax (Foxy) via rosbridge, so gap_explorer running on the laptop can drive the kobuki running on baymax.

| Machine | IP | ROS Distro | Role |
|---------|-----------|--------|--------------------------------------|
| Laptop  | 10.5.15.42 | Jazzy  | gap_explorer + rosbridge_server      |
| Baymax  | 10.5.10.74 | Foxy   | kobuki driver + bridge client        |

### Startup order

Start in this order: **B → A → C → D**. Driver first, then bridge server, then bridge client, then brain.

### Terminal A — Laptop (local)

Runs `rosbridge_server`, which exposes Jazzy topics over WebSocket on port 9090.
...
### Terminal B — Baymax (SSH from laptop)

Runs the kobuki driver, which subscribes to `/commands/velocity` on baymax.
...
### Terminal C — Baymax (SSH from laptop, second session)

Runs the bridge client. Pulls `/commands/velocity` from the laptop's rosbridge and republishes it locally on baymax's Foxy network.
...
### Terminal D — Laptop (local)

Runs `gap_explorer`, which publishes `/commands/velocity`.
...
After A, B, and C are running but before launching D, run this from any laptop terminal:
...
The kobuki should twitch forward briefly. If yes, the chain works end-to-end and it's safe to launch gap_explorer.

## Shutdown

Ctrl-C in reverse order: **D → C → A → B**.

## Troubleshooting

- **"rosbridge connection failed" on baymax (Terminal C):** the laptop's rosbridge isn't reachable. From baymax, run `nc -zv 10.5.15.42 9090`. If it fails, check that Terminal A is still running and that the laptop's IP hasn't changed (`ip addr show` on the laptop).
- **Bridge runs but kobuki doesn't move:** confirm both publisher and subscriber exist on baymax with `ros2 topic info /commands/velocity` (1 publisher = bridge, 1 subscriber = kobuki driver).
- **Laptop IP changed:** rerun the setup script with the new IP. The current laptop IP is hardcoded into `~/ros_bridge/cmd_vel_bridge.py` on baymax.
